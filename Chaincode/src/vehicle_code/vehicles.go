...
//==============================================================================================================================
//	Sku - Defines the structure for a Sku object. JSON on right tells it what JSON fields to map to
//			  that element when reading a JSON object into the struct e.g. JSON make -> Struct Make.
//================================================================================
type Sku struct {
	Make            string `json:"make"`
...
//==============================================================================================================================
//	SKU Holder - Defines the structure that holds all the skuIds for product that have been created.
//				Used as an index when querying all product.
//==============================================================================================================================

type Sku_Holder struct {
...
//				  method 'PutState'.
//redundant code needs to remove:Pragati
//==============================================================================================================================
...
//==============================================================================================================================
// save_change - Writes to the ledger the Vehicle struct passed in a JSON format. Uses the shim file's
//				  method 'PutState'.
//==============================================================================================================================
func (t *SimpleChaincode) save_change(stub shim.ChaincodeStubInterface, s Sku) (bool, error) {

	bytes, err := json.Marshal(s)

	if err != nil {
		fmt.Printf("SAVE_CHANGE: Error converting Sku record: %s", err)
		return false, errors.New("Error converting Sku record")
	}

	err = stub.PutState(s.SkuID, bytes)

	if err != nil {
		fmt.Printf("SAVE_CHANGE: Error storing Sku record: %s", err)
		return false, errors.New("Error storing Sku record")
	}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "create_Sku" {
		return t.create_Sku(stub, caller, caller_affiliation, args[0])
...
//=================================================================================================================================
func (t *SimpleChaincode) create_Sku(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string, skuID string) ([]byte, error) {
	var sku Sku

	SkuID := "\"skuID\":\"" + skuID + "\", " // Variables to define the JSON
	vin := "\"VIN\":0, "
	make := "\"Make\":\"UNDEFINED\", "
	...
	sku_json := "{" + SkuID + vin + make + model + reg + owner + colour + leaseContract + status + scrapped + "}" // Concatenates the variables to create the total JSON object

	matched, err := regexp.Match("^[A-z][A-z][0-9]{7}", []byte(skuID)) // matched = true if the skuID passed fits format of two letters followed by seven digits

	if err != nil {
		fmt.Printf("CREATE_SKU: Invalid skuID: %s", err)
		return nil, errors.New("Invalid skuID")
	}

	if skuID == "" ||
		matched == false {
		fmt.Printf("CREATE_SKU: Invalid skuID provided")
		return nil, errors.New("Invalid skuID provided")
	}

	err = json.Unmarshal([]byte(sku_json), &sku) // Convert the JSON defined above into a sku object for go

	if err != nil {
		return nil, errors.New("Invalid JSON object")
	}

	record, err := stub.GetState(sku.SkuID) // If not an error then a record exists so cant sku a new car with this assetID as it must be unique
	...
	if caller_affiliation != AUTHORITY { // Only the regulator can create a new v5c
		...
		return nil, errors.New(fmt.Sprintf("Permission Denied. create_Asset. %v === %v", caller_affiliation, AUTHORITY))

	}

	_, err = t.save_change(stub, sku)

	if err != nil {
		fmt.Printf("CREATE_SKU: Error saving changes: %s", err)
		return nil, errors.New("Error saving changes")
	}

	bytes, err := stub.GetState("skuID")

	if err != nil {
		return nil, errors.New("Unable to get assetID")
	}

	var skuIDs Sku_Holder

	err = json.Unmarshal(bytes, &skuIDs)

	if err != nil {
		return nil, errors.New("Corrupt Sku_Holder record")
	}

	skuIDs.Skus = append(skuIDs.Skus, SkuID)

	bytes, err = json.Marshal(skuIDs)

	if err != nil {
		fmt.Print("Error creating Sku_Holder record")
	}

	err = stub.PutState("skuIDs", bytes)
